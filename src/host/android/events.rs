...
//!
//! Touch events arrive from `nativeOnTouch` as `(action: i32, x: f32, y: f32)`. Action codes
//! are Android's `MotionEvent.ACTION_*`:
...
//!
//! Key events arrive from `nativeOnKeyEvent` as Android `KeyEvent` keycodes. Only the codes
//! photon currently routes are translated; everything else maps to `Key::Unidentified` so the
//! JNI layer can short-circuit and return `false` (unhandled).
//!
//! Text input arrives from `nativeOnTextInput` as a Java `String`. Translates to `Event::Ime`
//! with the committed text; the focused textbox's `Key` impl already ignores the keystroke
//! when an IME commit is active — apps just see the IME variant.
...
/// Translate an Android touch (`action`, x, y) into the fluor event stream the host loop
/// should dispatch in order. Returns a stack-allocated array of up to two events (the longest
/// translation is DOWN → CursorMoved + MouseInput, and CANCEL → CursorLeft + MouseInput).
///
...
// ----------------------------------------------------------------------------
// Key codes — Android `KeyEvent.KEYCODE_*`.
// Values from <android/keycodes.h>. Only what photon uses is mapped; extending the set is
// safe and additive.
...
/// Translate an Android `KeyEvent` keycode to a fluor [`Key`]. `Unidentified` for anything
/// we don't model — the JNI shim should treat that as "host doesn't handle this key" and
/// return false from `nativeOnKeyEvent` so Android's default behaviour kicks in.
...
/// Convenience: build a `KeyboardInput` press event from an Android keycode. Returns
/// `None` for unmapped keys (caller treats as unhandled).
