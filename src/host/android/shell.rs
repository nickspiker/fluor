...
impl<A: FluorApp> AndroidShell<A> {
    ...
    pub fn new(mut app: A, width: u32, height: u32) -> Self {
        ...
        // The surface NEVER resizes for the IME (the Activity runs adjustNothing and does no layoutParams surgery — the keyboard composites over it and the app lifts its bottom strips via the reported inset), so the live harmonic mean IS the stable full-screen scale: no pinned basis needed. Rotation swaps w/h, and 2wh/(w+h) is symmetric, so even that preserves the scale exactly.
    ...
    /// Resize the surface + viewport + scratch + clip_mask. Called from `nativeResize` on surfaceChanged — which now fires ONLY on creation, rotation, and real window resizes (split-screen): the IME never resizes the surface (adjustNothing + inset reporting). So the plain live harmonic mean is always the correct scale; rotation preserves it exactly (span is symmetric in w/h).
