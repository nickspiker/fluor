...
//! # Why this exists
//!
//! macOS separates the APPLICATION from its windows. A Dock icon represents the app, so clicking it is not
//! a window operation and produces no window event. AppKit instead asks the application's delegate
//! `applicationShouldHandleReopen:hasVisibleWindows:`. Nothing answered it, so a Dock click on a minimized
//! Photon was silently dropped while cmd-Tab and Mission Control worked (those drive WINDOWS, which do
//! produce events).
//!
...
//!
//! Adds the selector to winit's EXISTING delegate class with `class_addMethod`. Purely additive: winit's
//! own methods are untouched, nothing is replaced, and the app keeps exactly one delegate. It no-ops safely
//! if the class is ever renamed or already answers the selector, so a winit upgrade degrades to today's
...
///
/// `extern "C"` with no unwinding across the boundary: this is called BY AppKit, so a panic here would
/// abort the process. It does nothing but set a flag, which cannot panic.
...
pub(super) fn install() {
    ...
    unsafe {
        ...
        // `Imp` is a bare `unsafe extern "C-unwind" fn()`; the cast erases our real signature, which is why
        // the `types` string above must describe it exactly ("B@:@B" = BOOL result; self, _cmd, id, BOOL).
