...
//!
//! Single entry point [`par_rows`]: takes a contiguous `width × height` `u32` pixel buffer, a row range (clip-aware), and a closure invoked per row. With the `rayon` feature on, rows are dispatched across the rayon thread pool; off, the same iteration runs sequentially. The closure signature is identical either way — rasterizers and boundary passes don't branch on the feature gate, they just call this wrapper and the right thing happens.
//!
...
/// Iterate a flat `pixels` slice in equal-sized chunks of `chunk_len` (handles trailing tail of any length). Closure receives `(chunk_offset_in_pixels, &mut chunk_slice)`. Same Rayon-vs- sequential dispatch as [`par_rows`] — used for whole-buffer passes (finalize, flatten, Op::Under) where row geometry doesn't matter and chunking by a multiple of cache-line size gives the best throughput.
#[inline]
